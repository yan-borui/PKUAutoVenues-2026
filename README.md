...
3. 等待到 12 点，开始预约；12:00 最多尝试 8 次，如果仍未成功，再等待 12:11:00、12:12:00、12:13:00 抢可能因未付款而释放的回流场，每个回流时间窗口也各自最多尝试 8 次：
   - 放号后获取验证码并识别、校验；校验成功后保存验证码图片，再查询场地。识别或校验失败时也会保存图片，便于排查
   - 按 `venue` 参数顺序获取 `date` 日期各场馆的预约信息，筛选出每个场地的空闲时段；前一个场馆有候选时不会请求后续场馆
   - 根据 `times` 和 `spaces` 优先级，决定本轮要尝试预约哪个场地的哪个（哪些）时段
   - 确保距离验证码校验成功至少经过 1 秒再提交；图片存盘和场地查询耗时计入这一间隔，仅等待剩余时间。历史运行中，提前获取的验证码曾在 12 点后失效，校验后过快提交也曾触发验证码错误，因此保留这两项保护
...
严格等待放号时，最后 100 毫秒内每次休眠最多 10 毫秒；每次休眠都不超过剩余等待时间，醒来后重新检查时间，到点才发起请求。

日志默认记录 INFO 及以上级别，保留关键过程、结果和错误；请求参数、响应详情、Cookie 和分隔空行默认不写入日志，也不进行相应的遍历与格式化。需要排查时，在原命令后追加 `--debug` 即可恢复详细文件日志，控制台仍保持 INFO，日志继续脱敏。验证码图片在两种模式下都会保存到 `logs/`。

日志时间戳精确到毫秒。每次尝试结束后输出一条 `Attempt timing (ms)` 摘要，包括结果 `outcome` 和已执行阶段的耗时：

| 字段 | 含义 |
| --- | --- |
| `captcha_get` / `recognize` / `captcha_check` | 验证码获取、识别和校验耗时 |
| `captcha_save` / `availability` | 验证码图片存盘、场地查询与候选筛选耗时 |
| `captcha_wait` / `submit` | 补足保护间隔的实际等待、提交调用耗时 |
| `order_recovery` | 提交结果不确定时，查询未支付订单和等待网站恢复的耗时，仅在发生时记录 |
| `total` | 本次尝试的总耗时，包含阶段之间的本地处理与日志开销 |
| `submit_after_release` | 开始调用提交接口时，相对目标日期对应的首次放号时间（12:00）的偏移；首条包含该字段的摘要记录首次提交偏移，回流窗口沿用同一基准 |

阶段耗时使用单调时钟；提交偏移由尝试开始时的北京时间与后续单调时钟差值计算。失败尝试也会记录已执行阶段的耗时，未开始的阶段省略。涉及 HTTP 的阶段包含请求及本地处理耗时，可结合存盘、等待和总耗时分析下一次正式运行的延迟；这些记录不代表已经测得提速幅度。
